      ...
      def generate_html_map(json_map, sorted_stats, selected_stat, timestamp_index)
        ...
        json_map["chartshape"].each do |area|
          ...
          if area["name"] =~ /axis2_(.+)/
            ...
            value = number_with_delimiter item.get_value selected_stat, timestamp_index, true
            title = item.id  #this may be an actual name later
            href = item.is_a?(RangeServer) ? "/rangesevers" : "/tables" #title is also id right now. todo: better way to determine the path?
